...
{
    /// <summary>
    /// Tab Di chuyển (Migrate) — toàn bộ giao diện di chuyển trước đây của cửa sổ chính:
    ...
    /// </summary>
    public sealed class MigrateTabPage : UserControl
    {
        ...
        public MigrateTabPage(IDataProtector protector, IConnectionProfileStore profileStore, SecureConnectionStringBuilder builder)
        {
            Dock = DockStyle.Fill;
        ...
        {
            // Thuộc UserControl: chỉ co giãn + font (kích thước/chữ form do shell giữ).
        ...
        private void BuildLayout()
        {
            ...
            // UserControl không có sự kiện Shown của Form nên dùng Load.
            Load += (_, _) =>
            {
...
var numbers = new FlowLayoutPanel { Dock = DockStyle.Bottom, WrapContents = false, AutoSize = true };
        ...
        /// <summary>Nạp icon app từ resource nhúng (rớt êm nếu thiếu). Shell dùng chung.</summary>
